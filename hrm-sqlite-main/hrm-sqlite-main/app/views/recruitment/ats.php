...
<div class="card p-3 mb-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        <div>
            <h4 class="mb-0"><i class="bi bi-kanban text-primary me-2"></i> Applicant Tracking System (ATS)</h4>
            <div class="text-muted small">Manage candidates across all hiring stages from application to onboarding.</div>
        </div>
    ...
    <div class="d-flex flex-wrap gap-2 pt-2 border-top">
        <a href="<?php echo BASE_URL; ?>/recruitment/ats<?php echo $selectedJobId ? '?job_id=' . $selectedJobId : ''; ?>" 
           class="btn btn-sm <?php echo empty($selectedStage) ? 'btn-dark' : 'btn-outline-dark'; ?>">
            All (<?php echo array_sum($stageCounts); ?>)
        </a>
    ...
    <form method="GET" action="<?php echo BASE_URL; ?>/recruitment/ats" class="row g-2 align-items-center">
        ...
        <div class="col-md-5">
            <div class="input-group input-group-sm">
                ...
                <input type="text" name="q" value="<?php echo htmlspecialchars($keyword ?? ''); ?>" class="form-control" placeholder="Search by name, email, phone, company...">
            </div>
        </div>
            ...
            <select name="job_id" class="form-select form-select-sm" onchange="this.form.submit()">
                ...
                <?php foreach ($jobs as $j): ?>
                    <option value="<?php echo $j['id']; ?>" <?php echo $selectedJobId == $j['id'] ? 'selected' : ''; ?>>
                        <?php echo htmlspecialchars($j['title'] . ' (' . $j['status'] . ')'); ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="col-md-2 d-flex gap-1">
            ...
            <?php if (!empty($selectedJobId) || !empty($selectedStage) || !empty($keyword)): ?>
                <a href="<?php echo BASE_URL; ?>/recruitment/ats" class="btn btn-sm btn-outline-secondary" title="Reset Filters"><i class="bi bi-arrow-counterclockwise"></i></a>
            <?php endif; ?>
        </div>
    </form>
</div>
                ...
                <?php if (empty($candidates)): ?>
                    <tr>
                        <td colspan="7" class="text-center text-muted py-4">
                            No candidates found in this stage or filter.
                            <br><a href="<?php echo BASE_URL; ?>/recruitment/addCandidate<?php echo $selectedJobId ? '?job_id=' . $selectedJobId : ''; ?>" class="btn btn-sm btn-outline-primary mt-2">Add Candidate</a>
                        </td>
                    </tr>
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="text-muted small"><?php echo htmlspecialchars($c['department_name'] ?? 'Unassigned'); ?></div>
                        </td>
                        <td>
                            <?php if ($c['experience_years'] > 0): ?>
                                <span class="badge bg-light text-dark border"><?php echo $c['experience_years']; ?> yrs</span>
                            <?php else: ?>
                        ...
                        <td>
                            <span class="badge bg-<?php echo $stageMeta['badge']; ?> d-inline-flex align-items-center gap-1">
                                <i class="bi <?php echo $stageMeta['icon']; ?>"></i> <?php echo $c['stage']; ?>
                            </span>
                        </td>
                        <td class="text-muted small">
                            <?php echo date('M j, Y', strtotime($c['applied_date'])); ?>
                        </td>
                        <td class="text-end text-nowrap">
                            <a href="<?php echo BASE_URL; ?>/recruitment/candidate/<?php echo $c['id']; ?>" class="btn btn-sm btn-outline-secondary" title="View Candidate Profile">
                                <i class="bi bi-eye"></i> View
                            </a>

                            <!-- Hire & Onboard Button -->
                            <?php if (in_array($c['stage'], ['Offered', 'Hired']) && empty($c['hired_employee_id'])): ?>
                                <a href="<?php echo BASE_URL; ?>/onboarding/create?candidate_id=<?php echo $c['id']; ?>" class="btn btn-sm btn-success" title="Hire and Start Onboarding">
                                    <i class="bi bi-person-check"></i> Hire & Onboard
                                </a>
                            <?php elseif (!empty($c['hired_employee_code'])): ?>
                                <span class="badge bg-success-subtle text-success small border border-success ms-1">
                                    <i class="bi bi-check-circle"></i> Onboarded (<?php echo htmlspecialchars($c['hired_employee_code']); ?>)
                                </span>
                            <?php endif; ?>

                            <!-- Quick Stage Dropdown -->
                            <div class="dropdown d-inline ms-1">
                                <button class="btn btn-sm btn-outline-primary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false" title="Move Stage">
                                    Move
                                </button>
                                <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                                    <li class="dropdown-header small">Advance Stage</li>
