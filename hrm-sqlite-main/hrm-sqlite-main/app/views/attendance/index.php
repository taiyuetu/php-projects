<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <form class="d-flex gap-2" method="GET" action="<?php echo BASE_URL; ?>/attendance">
        ...
        <button class="btn btn-outline-secondary"><i class="bi bi-filter"></i> 筛选</button>
    </form>
</div>
...
<?php if ($_SESSION['user_role'] === 'employee'): ?>
    <div class="mb-3">
        <form method="POST" action="<?php echo BASE_URL; ?>/attendance/check-in" class="d-inline">
            ...
            <button class="btn btn-success"><i class="bi bi-box-arrow-in-right"></i> 签到</button>
        </form>
        <form method="POST" action="<?php echo BASE_URL; ?>/attendance/check-out" class="d-inline">
            <input type="hidden" name="_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'] ??= bin2hex(random_bytes(32))); ?>">
            <button class="btn btn-outline-danger"><i class="bi bi-box-arrow-right"></i> 签退</button>
        </form>
    </div>
<?php endif; ?>
            ...
            <tbody>
                ...
                <?php else: foreach ($attendance as $row): ?>
                    <tr>
                        ...
                        <td><span class="badge badge-status-<?php echo str_replace(' ', '.', $row['status']); ?>"><?php echo htmlspecialchars($attStatusMap[$row['status']] ?? $row['status']); ?></span></td>
                    </tr>
