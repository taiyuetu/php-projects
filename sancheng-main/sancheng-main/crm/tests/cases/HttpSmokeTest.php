...
function withTestServer(string $tag, callable $fn): void
{
    ...
    // config.php 读 BASE_PATH/.env，所以 web 子进程与本进程共用这个测试库。
    // UPLOAD_PATH / BACKUP_PATH 也一起写进去：备份用例会在子进程里真的写 zip、解包、覆写库，
    // 只靠“环境变量会继承给子进程”隔不干净（实测曾污染到仓库的 database/backups/）。
    // .env 是子进程确定会读的那一份；两边都给，以防万一。
    file_put_contents($envFile, 'DB_PATH=' . $GLOBALS['TEST_DB_PATH'] . PHP_EOL
        . 'UPLOAD_PATH=' . $GLOBALS['TEST_UPLOAD_PATH'] . PHP_EOL
        . 'BACKUP_PATH=' . $GLOBALS['TEST_BACKUP_PATH'] . PHP_EOL);
...
function test_all_main_pages_respond_200(): void
{
    ...
    withTestServer('smoke', function (TestHttp $http, string $base, string $csrf): void {
        ...
        $pages = [
            ...
            // 备份页（管理员）：只证它能渲染，真打包/真恢复在下面的往返用例里跑
            '/backup',
...
/**
 * 备份的 HTTP 往返：登录 → 看页 → 导出（真下载）→ 生成快照 → 改数据 →
 * 错确认词导入（必须无效）→ 对确认词导入（数据真的回到过去）。
 *
 * 这一步值得真跑 HTTP：库文件覆写发生在“本请求已经持着这个连接”的时刻，
 * 单元测试里拿不到的就是这个并发场景（Windows 上尤其）。
 */
function test_backup_export_and_restore_round_trip_over_http(): void
{
    withTestServer('backup', function (TestHttp $http, string $base, string $csrf): void {
        $page = $http->get($base . '/backup');
        assertEquals(200, $page['code'], '备份页对管理员渲染 200');
        assertContains('备份与恢复', $page['body'], 'and it is the backup page');
        assertContains('覆盖导入', $page['body'], 'the confirmation word is shown to be typed');
        assertContains('/backup/import', $page['body'], 'the import form is wired');
        assertTrue(str_contains($page['body'], 'backup_file'), 'the upload control exists');
        // 这页是我手写的卡片表格，开合不配平不会报错，只会在浏览器里把后面的页弄坏
        assertTrue(substr_count($page['body'], '<div') === substr_count($page['body'], '</div>'),
            'backup page HTML is balanced (' . substr_count($page['body'], '<div') . '/'
            . substr_count($page['body'], '</div') . ')');
        assertTrue(!str_contains($page['body'], '<?'), 'backup page leaks no raw PHP');
        assertTrue(!str_contains($page['body'], 'Warning:') && !str_contains($page['body'], 'Deprecated:'),
            'and no PHP notice lands in the markup');

        // 1) 导出：真字节流回手里，而且必须是 zip
        $export = $http->post($base . '/backup/export', [
            'csrf_token' => $csrf, 'with_database' => '1', 'with_uploads' => '1', 'note' => 'HTTP 往返',
        ]);
        assertEquals(200, $export['code'], 'export returns 200, not a redirect');
        assertContains('attachment', implode(' ', $http->lastHeaders['content-disposition'] ?? []),
            'sent as a file download');
        assertTrue(str_starts_with($export['body'], 'PK'), 'the body is really a zip (PK magic)');
        assertTrue(strlen($export['body']) > 500, 'and it carries content, not an empty archive');

        // 2) 服务器快照：给下面的恢复用（也是“不受上传限制”的那条路径）
        // TestHttp 会跟着 302 落到 /backup，所以直接看回页里的提示。
        $snap = $http->post($base . '/backup/snapshot', [
            'csrf_token' => $csrf, 'with_database' => '1', 'with_uploads' => '1', 'keep' => '10',
        ]);
        assertContains('快照已生成', $snap['body'], 'the redirect reports the snapshot it made');
        $names = array_column(Backup::listSnapshots(), 'name');
        assertTrue($names !== [], 'the snapshot is on disk where the page can see it');
        assertContains($names[0], $snap['body'], 'and the name in the message is the file itself');

        // 3) 备份之后改数据：恢复必须把它抹掉，否则“回滚”是假的
        (new Customer())->create(['name' => '恢复前新增的客户', 'owner_id' => 1]);
        $withExtra = (int) (new Customer())->count();
        assertTrue($withExtra > 0, 'the extra customer is in');

        // 4) 确认词不对：必须不动数据（这一条防的是“自动化脚本一句话把全站换掉”）
        // 注：TestHttp 会跟着 302 再 GET 一次，所以 flash 已在 $bad['body'] 里被消耗掉了，
        // 不要指望下一个请求还能看到它。
        $bad = $http->post($base . '/backup/import', [
            'csrf_token' => $csrf, 'source_kind' => 'server', 'snapshot' => $names[0],
            'with_database' => '1', 'confirm_text' => '我确定',
        ]);
        assertEquals($withExtra, (int) (new Customer())->count(), '打错确认词时一行不少');
        assertContains('未确认', $bad['body'], '并把原因告诉了管理员');

        // 5) 确认词对：数据真的回到快照时刻
        $ok = $http->post($base . '/backup/import', [
            'csrf_token' => $csrf, 'source_kind' => 'server', 'snapshot' => $names[0],
            'with_database' => '1', 'with_uploads' => '1', 'confirm_text' => '覆盖导入', 'no_migrate' => '1',
        ]);
        assertEquals($withExtra - 1, (int) (new Customer())->count(), '快照之后新增的客户真的不见了');
        assertContains('导入完成', $ok['body'], '页面报出了完成的导入');
        assertContains('本次导入结果', $ok['body'], '并展开成可对账的明细卡');
        assertContains('pre-restore', $ok['body'], '明细里点名了能反悔的那个回滚快照');

        // 6) 回滚点本身也必须能当来源：刚导错的那一刻，它不该被 upload_max_filesize 拦在门外
        //（曾经这个下拉只列 crm-backup-*，回滚点能下载不能选，等于把最短的退路堵了）
        $pageAfter = $http->get($base . '/backup')['body'];
        assertTrue(str_contains($pageAfter, 'pre-restore'), 'the rollback point is listed');
        $rollback = null;
        foreach (array_column(Backup::listSnapshots(), 'name') as $n) {
            if (str_contains($n, 'pre-restore')) { $rollback = $n; break; }
        }
        assertTrue($rollback !== null, 'a rollback point exists on disk');
        // 必须看来在 <option> 里，不是看页面上出现过这个名字 —— 表格行也印它，
        // 不这么框住的话，这个断言会在下拉回归时依然通过（假绿比没有测试更糟）。
        preg_match_all('~<option value="([^"]+)"~', $pageAfter, $opts);
        assertTrue(in_array($rollback, $opts[1], true), 'and it is selectable as a restore source');
        assertTrue(str_contains($pageAfter, '回滚点·仅数据库'),
            'labelled as database-only, so nobody expects attachments back');

        // 7) 不存在的快照名不能被拿来恢复（路径参数只过白名单）
        $evil = $http->post($base . '/backup/import', [
            'csrf_token' => $csrf, 'source_kind' => 'server', 'snapshot' => '../../.env',
            'with_database' => '1', 'confirm_text' => '覆盖导入',
        ]);
