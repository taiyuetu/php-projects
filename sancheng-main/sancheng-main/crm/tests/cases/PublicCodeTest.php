...
function codeOldDb(string $tag): string
{
    ...
    $before = $schema;
    // 造一个“v1.6 时代的库”：三张业务表还没有 public_code，商品库与明细外键也还没有。
    // 只能按表剥列 —— 全局剥 public_code 会把 products 的列也剥掉，
    // 而 order_items 的外键指向一张缺列的表，SQLite 在插种子时就会报“no such table”。
    foreach (['customers', 'leads', 'deals'] as $t) {
        $schema = preg_replace('~(CREATE TABLE IF NOT EXISTS ' . $t . ' \(.*?\n\);)~s',
            '$1', $schema, 1, $cnt);
        $schema = preg_replace_callback('~CREATE TABLE IF NOT EXISTS ' . $t . ' \((.*?)\n\);~s',
            static fn(array $m) => 'CREATE TABLE IF NOT EXISTS ' . $t . ' ('
                . preg_replace('~^\s*public_code\s+TEXT,\s*$~mi', '', $m[1]) . "\n);",
            $schema, 1);
    }
    $schema = preg_replace('~-- 2b\. Products.*?(?=-- 3\. Leads)~s', '', $schema);
    $schema = preg_replace('~-- 1c\. Sample products.*?(?=-- 2\. Sample customers)~s', '', $schema);
    // 老库的 order_items：没有 product_id 列，也没有指向 products 的外键
    $schema = preg_replace('~^\s*-- product_id[^\n]*\n~m', '', $schema);
    $schema = preg_replace('~^\s*product_id\s+INTEGER,\s*\n~m', '', $schema);
    $schema = preg_replace('~^\s*FOREIGN KEY \(product_id\)[^\n]*\n~m', '', $schema);
    // 删掉 product_id 外键后，它前一行（orders 外键）的结尾逗号就落在 `)` 前面了 —— 不补一下就是
    // `near ")": syntax error`。写这段时用的是 `,...\n` 字面量，而 schema.sql 在 Windows 工作区里
    // 是 CRLF（git 索引里是 LF），所以这个替换在开发机上静默失效 —— 改成像上面几条那样用正则，
    // 两种行尾都得成立（同一个坑不再靠记性避）。不匹到真正的基线，只改这份造出来的夹具。
    $schema = preg_replace('~(REFERENCES orders\(id\) ON DELETE CASCADE),[ \t]*\r?\n~', "$1\n", $schema);
    // 注释里提一句的列存在与否不影响，只要语句真少了这一列
    assertTrue($schema !== $before, '测试本身要能剥掉 public_code');
    assertTrue(!str_contains($schema, 'CREATE TABLE IF NOT EXISTS products'), '夹具里不该还有商品库');
