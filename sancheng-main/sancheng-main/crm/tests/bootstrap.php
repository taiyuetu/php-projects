...
// ---- Isolated backup & attachment dirs ----
// 备份/恢复用例会真的打包、解包、覆写文件（还会往附件目录里写）。不能让它们在仓库
// 里留下 .zip，更不能碰到 public/uploads/attachments 里的真附件 —— 所以和 DB_PATH 一样，
// 在 require config.php 之前先把这两个目录指到临时区（config.php 里真实环境变量优先于 .env）。
$uniq = getmypid() . '_' . bin2hex(random_bytes(3));
$testUploadDir = sys_get_temp_dir() . '/crm_test_up_' . $uniq;
$testBackupDir = sys_get_temp_dir() . '/crm_test_bk_' . $uniq;
mkdir($testUploadDir, 0777, true);
mkdir($testBackupDir, 0777, true);
putenv('UPLOAD_PATH=' . $testUploadDir);
putenv('BACKUP_PATH=' . $testBackupDir);
$_ENV['UPLOAD_PATH'] = $testUploadDir;
$_ENV['BACKUP_PATH'] = $testBackupDir;
$GLOBALS['TEST_UPLOAD_PATH'] = $testUploadDir;
$GLOBALS['TEST_BACKUP_PATH'] = $testBackupDir;
...
function runCase(): void
{
    ...
    // 备份用例会在临时目录里留下包、快照与附件；一并扫掉，不把 temp 塞满
    foreach (['TEST_UPLOAD_PATH', 'TEST_BACKUP_PATH'] as $key) {
        $dir = $GLOBALS[$key] ?? '';
        if ($dir && is_dir($dir)) {
