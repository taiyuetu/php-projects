...
// Overridable for the same reason DB_PATH is: hosting layouts differ, and the
// backup tests must not write into the real attachment folder. Relative paths
// resolve from BASE_PATH.
$absPath = static function (string $path): string {
    if ($path !== '' && ($path[0] !== '/' && strpos($path, ':') === false)) {
        return BASE_PATH . '/' . ltrim($path, '/');
    }
    return $path;
};
define('UPLOAD_PATH', $absPath(getenv('UPLOAD_PATH') ?: 'public/uploads/attachments'));
define('BACKUP_PATH', $absPath(getenv('BACKUP_PATH') ?: 'database/backups'));
