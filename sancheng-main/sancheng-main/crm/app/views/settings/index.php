                ...
                <div class="row g-3">
                    <?php foreach ($definitions as $key => $def): ?>
                        ...
                        <div class="col-md-6">
                            ...
                            <?php endif; ?>
                            <?php if ($key === 'ai_max_tokens' && !empty($providers[$current]['tokens_floor'])): ?>
                                <div class="form-text">该服务商强制开启思考，思考过程也计入回复上限：这里填得比
                                    <strong><?= (int) $providers[$current]['tokens_floor'] ?></strong> 小会按
                                    <strong><?= (int) $providers[$current]['tokens_floor'] ?></strong> tokens 发送
                                    （否则思考阶段就把额度用完，只能拿到空回复）。</div>
                            <?php endif; ?>
