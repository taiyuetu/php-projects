...
<form method="POST" action="<?= e($action ?? url('/products')) ?>" class="card card-table p-4">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-3">
            <label class="form-label">SKU / 内部编号</label>
        ...
        <div class="col-md-3">
            ...
            <?php
            // 当前分类若已停用也要能看见（optionsFor 会把当前值补回选项并标注），
            // 避免编辑时下拉悄悄变回“无分类”。
            $curCatId = (int) ($old['category_id'] ?? 0);
            $catOptions = Category::optionsFor($curCatId ?: null, (string) ($old['category'] ?? ''));
            ?>
            <select name="category_id" class="form-select">
                <option value="">-- 无分类 --</option>
                <?php foreach ($catOptions as $id => $name): ?>
                    <option value="<?= e($id) ?>" <?= $curCatId === $id ? 'selected' : '' ?>>
                        <?= e($name) ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <div class="form-text"><a href="<?= url('/categories') ?>">管理分类</a></div>
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-6">
            <label class="form-label">规格 / 尺寸</label>
        ...
        <div class="col-md-3">
            <label class="form-label">销售价 <span class="text-danger">*</span></label>
