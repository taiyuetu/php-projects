...
class Ai extends Model
{
    ...
    const PROTECTED_COLUMNS = [
        ...
        'products'   => ['category_id'],   // FK 内部编号：AI 照旧写 category 文本，不猜数字 id
        'leads'      => ['lost_at'],
    ...
    /**
     * 同一个计划里的“先建后用”：写 create_* 之后，后面的动作可以用 @N 引用第 N 步新建的那条。
     *
     * 为什么需要它：客户的询盘大多带来一个**还不存在的客户**，而商机必须挂在客户下。
     * 没有这个机制时，模型只能二选一：要么被“客户 找不到对应记录”拦死（用户明明说了“新建商机”），
     * 要么猜一个数字——猜中就错挂到别人名下，没猜中就撞上 FOREIGN KEY 报错。
     * 现在它出两个动作，预览页上“#1 新建客户 + #2 新建商机（客户：@1=第1步新建的客户）”人人看得懂，
     * 而编号是执行时由服务器填的，模型无法伪适。只允许向前引用（@N 必须小于当前动作序号）。
     *
     * @var array<string,string> 工具 => kindInfo 的 kind
     */
    private const CREATE_TOOLS = [
        'create_customer' => 'customer', 'create_lead' => 'lead', 'create_deal' => 'deal',
        'create_product'  => 'product', 'create_category' => 'category', 'add_follow_up' => 'follow_up',
    ];

    /** 引用类型 => 它能被哪个 create_* 满上 */
    private const REF_KIND = ['customer_id' => 'customer', 'lead_id' => 'lead', 'deal_id' => 'deal',
                              'order_id' => 'order', 'product_id' => 'product'];

    /**
     * 把 args 里的 @N 换成那一步真的新建出来的 id。换不到就原样留着——
     * 校验已经保证只会有合法且已成功的引用，这里不做第二次判断。
     */
    private static function bindPending(array $args, array $pending): array
    {
        if (!$pending) {
            return $args;
        }
        foreach ($args as $key => $value) {
            $want = self::pendingRef($value);
            if ($want !== null && isset($pending[$want])) {
                $args[$key] = (string) $pending[$want]['id'];
            }
        }
        return $args;
    }
    ...
    /**
     * 模型想用“刚建的线索”去挂商机时，把那一步改写成新建客户。
     *
     * 为什么在服务器上改、不靠提示词祈祷：商机外键只认客户，而真实模型面对
     * 「…询价…新建商机」常常两步都想要 —— 先 create_lead，再让商机写
     * customer_id:"@1" 挂上去，于是整批被类型校验拒掉（实测报错：
     * 「客户 的 @1 新建的是线索，与这里的类型不匹配」），用户看到的就是“建商机失败”。
     * 而本系统“线索转商机”按钮的语义正是“从线索的联系方式建一个客户，再挂商机”，
     * 所以这里做的是页面早就在做的事，不是发明新规矩：那一步由 create_lead 变成
     * create_customer，**序号不变**（后面的 @1 仍然指它），联系方式（含微信）一并带过去。
     * 代价是这一条不再留线索记录 —— 预览页写清楚了，用户在确认闸门上看得到。
     *
     * @return array{0:array<int,array>,1:string} [改写后的计划, 给人看的说明（未改写为空）]
     */
    public static function routeLeadIntoCustomerForDeal(array $actions): array
    {
        $rows = array_values($actions);
        $wanted = [];
        foreach ($rows as $i => $row) {
            if ((string) ($row['tool'] ?? '') !== 'create_deal') {
                continue;
            }
            $ref = self::pendingRef((string) (((array) ($row['args'] ?? []))['customer_id'] ?? ''));
            $target = $ref === null ? null : ($rows[$ref - 1] ?? null);
            if ($target !== null && (string) ($target['tool'] ?? '') === 'create_lead') {
                $wanted[$ref - 1] = true;
            }
        }
        if (!$wanted) {
        ...
        $note = '';
        foreach (array_keys($wanted) as $idx) {
            $args = (array) ($rows[$idx]['args'] ?? []);
            $title = textClip(trim((string) ($args['title'] ?? '')), 60);
            // customers.name 是必填列：联系人 → 公司 → 线索标题，三层兜底，总比整批失败好
            $name = trim((string) ($args['contact_name'] ?? '')) ?: trim((string) ($args['company'] ?? '')) ?: $title;
            $customer = [
                'name'     => $name,
                'company'  => (string) ($args['company'] ?? ''),
                'email'    => (string) ($args['contact_email'] ?? ''),
                'phone'    => (string) ($args['phone'] ?? ''),
                // 微信是这次才给线索补上的列：询盘最常只留微信，以前在线索阶段就丢了
                'wechat'   => (string) ($args['wechat'] ?? ''),
                ...
                'conversion_time' => appNow(),
            ];
            foreach (['first_purchase_from_china', 'has_import_capability'] as $flag) {
                if (isset($args[$flag]) && trim((string) $args[$flag]) !== '') {
                    $customer[$flag] = $args[$flag];
                }
            }
            $rows[$idx]['tool']  = 'create_customer';
            $rows[$idx]['args']  = array_filter($customer, static fn($v) => trim((string) $v) !== '');
            $rows[$idx]['reason'] = trim((string) ($rows[$idx]['reason'] ?? '')) . '（由线索改写：商机必须属于一个客户）';
            $note = '商机必须属于客户：已把第 ' . ($idx + 1) . ' 步的「新建线索」改写成「新建客户」'
                  . '（联系方式含微信一并带过去），商机挂在它下面；这条不再单独留线索记录，'
                  . '要进线索池请在确认前告诉我。';
        }
        return [$rows, $note];
    }
    ...
    /**
     * 预览页给人看的参数文字：@N 要说清它指向哪一步、那一步建的是什么，
     * 否则确认闸门上写的“客户: @1”等于让人闭着眼签。
     */
    public static function argText($value, array $plan = []): string
    {
        $raw = is_scalar($value) ? (string) $value : (string) json_encode($value, JSON_UNESCAPED_UNICODE);
        $want = self::pendingRef($value);
        if ($want === null) {
            return $raw;
        }
        foreach ($plan as $row) {
            if ((int) ($row['index'] ?? -1) === $want - 1) {
                $kind = self::CREATE_TOOLS[(string) ($row['tool'] ?? '')] ?? '';
                return $raw . '（用第 ' . $want . ' 步新建的'
                     . ($kind !== '' ? self::kindInfo($kind)['label'] : '记录') . '）';
            }
        }
        return $raw . '（第 ' . $want . ' 步不在本计划里）';
    }

    /** "@2" → 2；不是这个形式返回 null */
    private static function pendingRef($value): ?int
    {
        return is_string($value) && preg_match('/^@(\d{1,3})$/', trim($value), $m) === 1 ? (int) $m[1] : null;
    }
    ...
    public static function fieldsFor(string $table, bool $forCreate = false): array
    {
        ...
        foreach ($columns as $col) {
            ...
            // 但关联列（customer_id / deal_id 这类外键）是个例外：它们必须保留引用类型。
            // 这个例外是真bug逼出来的：Deal::$fields 把 customer_id 登记成 'int'，于是
            // create_deal 的参数被降级成普通整数 —— 而提示词叫模型“引用一律写 CUS-000007”，
            // 它就要么被“客户 必须是整数”拒掉，要么猜一个不存在的数字，然后在 execute 里撞出
            // SQLSTATE[23000] FOREIGN KEY constraint failed（人看到的就是一句数据库错误）。
            // 同一个列在 create_lead（leads 没注册表）上又是好的 —— 两张表两套规矩正是最坑的地方。
            if (isset($declared[$name]['type']) && !isset(self::LINK_COLUMNS[$name])) {
                $type = [
        ...
        return [
            ...
            'create_deal' => [
                ...
                'hint'   => 'stage 决定落在哪个看板列，阶段时间由系统写；要补充说明请用 add_follow_up。'
                            . '客户不存在时先 create_customer 再写 customer_id:"@1"（见规则 5b），不要猜编号，也不要改成建线索。',
...
1b. 主线是 线索 → 商机 → 客户 → 订单：询价/询盘/来信/报价这类首次接触，哪怕句子里写着“客户某某”，也一律先 create_lead（国家、联系人、需求写进线索）；句子里出现“建客户/客户档案/转客户/建商机/下单”这类明说时才能用 create_customer / create_deal。**点名要什么就做什么**：说“新建商机”就出 create_deal，别换成线索。
...
", $digest)) as $line) {
    ...
    public static function complete(string $instruction, ?int $userId = null): array
    {
        ...
        for ($round = 1; $round <= self::MAX_TOOL_ROUNDS; $round++) {
            ...
            // 上一条的反面：模型拿“刚建的线索”去挂商机（商机只认客户）→ 见方法注释
            [$rechain, $chainNote] = self::routeLeadIntoCustomerForDeal((array) ($parsed['actions'] ?? []));
            if ($chainNote !== '') {
                $parsed['actions'] = $rechain;
                $parsed['reply'] = trim((string) ($parsed['reply'] ?? '')) . ' ' . $chainNote;
            }
        ...
        foreach (array_values($actions) as $index => $action) {
            ...
            if (!isset($tools[$tool])) {
                ...
                foreach ($args as $name => $value) {
                    ...
                    $problem = self::checkArg($spec['params'][$name], $value, $userId, $index, $validated);
                    if ($problem !== null) {
                        $rowErrors[] = $problem;
                    }
                }
    ...
    /** @return array{actions:array<int,array>,errors:array<int,string>,blocked:bool} */
    private static function checkArg(array $spec, $value, int $userId, int $index = -1, array $plan = []): ?string
    {
        $label = $spec['label'];
        if ($value === null || $value === '') {
            if (!empty($spec['required'])) {
        ...
        switch ($spec['type']) {
            ...
            case 'product_id':
            case 'ai_action_id':
                // 先试“引用本计划前一步新建的那条”（@N），再走“库里已存在的记录”
                $want = self::pendingRef($value);
                if ($want !== null && isset(self::REF_KIND[$spec['type']])) {
                    return self::checkPendingRef($spec['type'], $want, $index, $plan, $label);
                }
     ...
     */
    /**
     * @N 只能指向“本计划里排在前面的一个 create_*”，而且类型要对得上。
     * 序号从 1 数（模型面向人），内部下标从 0 数；只许向前引用，不许指自己或后面那步，
     * 否则一个循环引用就能把整站数据绕过去。
     */
    private static function checkPendingRef(string $type, int $want, int $index, array $plan, string $label): ?string
    {
        if ($want < 1 || $want > $index) {
            return $label . ' 的 @' . $want . ' 无效：只能引用排在本动作之前的步骤（第 1 步起数，本动作是第 '
                 . ($index + 1) . ' 步）';
        }
        foreach ($plan as $row) {
            if ((int) ($row['index'] ?? -1) !== $want - 1) {
            ...
            $kind = self::CREATE_TOOLS[$tool] ?? null;
            if ($kind === null) {
                return $label . ' 的 @' . $want . ' 指的不是新建动作（' . ($row['label'] ?? $tool) . '），不能当编号用';
            }
            if ($kind !== (self::REF_KIND[$type] ?? '')) {
                return $label . ' 的 @' . $want . ' 新建的是' . self::kindInfo($kind)['label'] . '，与这里的类型不匹配';
            }
            if (!empty($row['errors'])) {
                return $label . ' 引用的第 ' . $want . ' 步本身有问题（先修好它）';
            }
            return null;
        }
        return $label . ' 的 @' . $want . ' 在本计划里找不到第 ' . $want . ' 步';
    }
    ...
    private static function checkRecordRef(string $type, $value, int $userId, string $label): ?string
    {
        ...
        if (!$row) {
            ...
            // 把“拿错了编号”与“真的查不到”分开说：商机挂客户时写了 LEAD-xxxx 是刚刚真实发生的错误，
            // 只说“找不到”会让模型接着换另一个编号再试一轮，而它需要知道的是“这两张表不一样”。
            $prefix = ['lead_id' => 'LEAD-', 'customer_id' => 'CUS-', 'deal_id' => 'DEAL-',
                        'order_id' => 'ORD-', 'product_id' => 'PROD-'][$type] ?? '';
            $other = ['LEAD-' => '线索', 'DEAL-' => '商机', 'ORD-' => '订单', 'PROD-' => '商品', 'CUS-' => '客户'];
            $wantTool = ['customer_id' => ['客户', 'create_customer'], 'lead_id' => ['线索', 'create_lead'],
                         'deal_id' => ['商机', 'create_deal'], 'order_id' => ['订单', 'create_order'],
                         'product_id' => ['商品', 'create_product']][$type] ?? [$label, 'create_*'];
            $up = strtoupper(trim((string) $ref));
            foreach ($other as $pre => $what) {
                if (str_starts_with($up, $pre) && $pre !== substr($prefix, 0, strlen($pre))) {
                    return "{$label}「" . textClip($ref, 30) . "」是{$what}的编号，这里要的是{$wantTool[0]}编号"
                         . "（形如 {$prefix}000007）。{$wantTool[0]}还不存在就同一个计划里分两步：先 {$wantTool[1]}，"
                         . "下一步这里写 \"@N\"（N=那一步的序号）；已经存在就先 search_records 查到真实编号";
                }
            }
    ...
    public static function execute(array $validatedActions, int $userId, int $selfActionId = 0): array
    {
        ...
        $applied = 0;
        $refused = 0;
        $pending = [];        // 步骤序号 => ['kind'=>..,'id'=>..,'code'=>..]，供后面的 @N 引用
        ...
        foreach ($validatedActions as $action) {
            ...
            $tool = (string) $action['tool'];
            try {
                // @N 在这一步才落地：前面哪个 create_* 成功了、它的真实 id 是多少，只有跑到这里
                // 才知道。编号由服务器填，模型猜不出来，也伪适不了。
                $args = self::bindPending((array) $action['args'], $pending);
                $outcome = self::runTool($tool, $args, $userId, $selfActionId);
            } catch (Throwable $e) {
                $outcome = ['ok' => false, 'message' => '执行失败：' . $e->getMessage()];
            }
            $outcome['tool'] = $tool;
            $outcome['label'] = self::toolLabel($tool);
            if ($outcome['ok'] && !empty($outcome['id'])) {
                // 记下这一步新建了哪条，给后面的 @N 用（序号面向人，从 1 数）
                $pending[((int) $action['index']) + 1] = [
                    'kind' => self::CREATE_TOOLS[$tool] ?? '',
                    'id'   => (int) $outcome['id'],
    ...
    private static function runModify(string $kind, array $args): array
    {
        ...
        // 订单状态在 cancelled 边界上翻转时，库存占用要跟着释放/恢复（明细本身不变）。
        // 页面编辑走 OrderItem::syncItems 的 prevStatus 差值；这里专门兜住 AI 单独改状态这一路。
        $orderStatusFlip = null;                          // null 不处理 | 'release' 进取消 | 'commit' 出取消
        if ($table === 'orders' && isset($data['status'])
            && (string) $data['status'] !== (string) ($before['status'] ?? '')) {
            $orderStatusFlip = $data['status'] === 'cancelled' ? 'release' : 'commit';
        }

        $ok = (bool) $model->update($id, $data);
        if ($orderStatusFlip === 'release') {
            OrderItem::releaseOrderStock($id, true);   // 状态已改成 cancelled，强制按行释放
        } elseif ($orderStatusFlip === 'commit') {
            OrderItem::commitOrderStock($id);
        }
    ...
    private static function runDelete(string $type, int $id, ?string $reason): array
    {
        ...
        if ($type === 'customer') {
            ...
            foreach (['orders' => '订单', 'deals' => '商机', 'leads' => '线索'] as $t => $name) {
                ...
                // 连带删的订单要把占用库存加回去（删完外键级联就来不及读了）
                if ($t === 'orders') {
                    foreach ($children as $cid) {
                        OrderItem::releaseOrderStock((int) $cid);
                    }
                }
                foreach ($children as $cid) {
            ...
            // 直接 SQL 删订单：先释放它占用的库存（不经过 Order::delete 覆写，必须在这处理）
            OrderItem::releaseOrderStock($id);
        } elseif ($type === 'deal') {
