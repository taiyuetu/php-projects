...
class Product extends Model
{
    ...
    protected static array $fields = [
        ...
        'sku'         => ['label' => 'SKU/内部编号', 'type' => 'string', 'searchable' => true,
                          'unique' => true, 'max' => 60, 'csv' => true],
        
        'category_id' => ['label' => '分类', 'type' => 'int', 'searchable' => false, 'csv' => true],
        ...
        'spec'        => ['label' => '规格尺寸', 'searchable' => true, 'csv' => true],
        'inventory'  => ['label'  => '库存', 'searchable' => true, 'csv' => true,
                          'form' => ['width' => 'col-md-3', 'placeholder' => '如：50',
                                     'hint' => '手动维护的现货量。销售订单保存/取消会自动增减；未填则不自动扣。'],
                          'list' => true],
        'car_make'  => ['label' => '制造商','searchable' => true,'csv' => true],
    ...
    /** 当前数值库存（未填/非数字按 0 计，仅用于计算，不改存储）。 */
    public function stockValue(int $id): float
    {
        $row = $this->db()->query('SELECT inventory FROM products WHERE id = :id')
            ->bind(':id', $id)->single();
        $raw = trim((string) ($row['inventory'] ?? ''));
        return $raw !== '' && is_numeric($raw) ? (float) $raw : 0.0;
    }

    /**
     * 库存增减 delta（可负）。只在"已设过数值库存"的商品上生效：
     * inventory 为 NULL/空 表示还没启用库存管理，此时不自动扣也不自动加；
     * 非数字历史文本同样跳过，绝不破坏既有数据。
     */
    public function adjustStock(int $id, float $delta): void
    {
        ...
        $next = round((float) $raw + $delta, 4);
        $this->db()->query('UPDATE products SET inventory = :v WHERE id = :id')
            ->bind(':v', (string) $next)
            ->bind(':id', $id)
            ->execute();
    }
    ...
    public function buildWhere(string $select, string $alias, string $search, string $status, string $category): array
    {
        ...
        if ($search !== '') {
            ...
            // 另补 public_code（系统编号）与 category（文本镜像列，触发器随分类同步，
            // 让“搜分类名能命中商品”的旧行为成立；只在商品查询这层补，不进全局 searchable）。
        ...
        if ($category !== '') {
            $where[] = "{$alias}.category_id = :category";
    ...
    public function resolve($ref): ?array
    {
        ...
        if ($id) {
            $row = $this->find($id);
            if ($row) {
                return $this->withCategoryName($row);
            }
        }
        ...
        $bySku = $this->db()->query('SELECT * FROM products WHERE LOWER(sku) = LOWER(:s) ORDER BY id LIMIT 1')
            ->bind(':s', $ref)->single();
        if ($bySku) {
            return $this->withCategoryName($bySku);
        }
        ...
        return count($rows) === 1 ? $this->withCategoryName($rows[0]) : null;
    }

    /**
     * 给商品行附加 category 字段（兼容旧代码）
     */
    private function withCategoryName(array $row): array
    {
        if (!isset($row['category']) && !empty($row['category_id'])) {
            $cat = $this->db()->query('SELECT name FROM categories WHERE id = :id')
                ->bind(':id', (int) $row['category_id'], PDO::PARAM_INT)->single();
            $row['category'] = $cat ? (string) $cat['name'] : '';
        }
        return $row;
    }
    ...
    public static function csvColumns(): array
    {
        ...
        foreach (static::$fields as $name => $meta) {
            ...
            $label = is_array($meta['csv']) ? ($meta['csv']['label'] ?? $meta['label'] ?? $name)
                                           : ($meta['label'] ?? $name);
            // category_id 在 CSV 中显示为 category（向后兼容）
            $csvName = $name === 'category_id' ? 'category' : $name;
            $out[$csvName] = (string) $label;
        }
        return $out;
    }
    ...
    private function csvCell(array $p, string $field): string
    {
        ...
        // category_id 导出为分类名称（CSV 中显示为 category）
        if ($field === 'category_id' || $field === 'category') {
            $categoryId = (int) ($p['category_id'] ?? 0);
            if ($categoryId > 0) {
                $cat = Database::connection()->prepare('SELECT name FROM categories WHERE id = ?');
    ...
    public function importCsvFile(string $path, int $ownerId): array
    {
        ...
        // CSV 中的 category 列对应数据库的 category_id
        if (in_array('category', $present, true)) {
            $present = array_values(array_diff($present, ['category']));
            $present[] = 'category_id';
        }
        $line = 1;                                                   // 表头占第 1 行
        while (($cells = fgetcsv($h, 0, $delim)) !== false) {
            ...
            foreach ($map as $field => $idx) {
                ...
                // CSV 中的 category 是文本，需要转换为 category_id
                if ($field === 'category') {
                    $categoryText = $v;
                    continue;
                }
            ...
            // 将分类文本转换为 category_id
            if ($categoryText !== '') {
                $categoryId = self::resolveCategoryId($categoryText);
                if ($categoryId) {
            ...
            if ($selfId > 0) {
                ...
                $merged = $input;
                foreach (['name', 'sku', 'partnumber', 'category_id', 'brand', 'spec', 'unit', 'price', 'cost', 'status', 'notes'] as $f) {
                    if (($merged[$f] ?? '') === '') {
                        $old = $existing[$f] ?? '';
    ...
    /**
     * 分类名称 → 分类 ID；如果分类不存在则自动创建。
     * CSV 导入时用，返回 null 表示分类名为空。
     */
    private static function resolveCategoryId(string $name): ?int
    {
        ...
        $stmt = $db->prepare('SELECT id FROM categories WHERE LOWER(name) = LOWER(:n) LIMIT 1');
        ...
        // 分类不存在，自动创建
        $stmt = $db->prepare("INSERT INTO categories (name, status, created_at, updated_at) VALUES (:n, 'active', datetime('now'), datetime('now'))");
        $stmt->bindValue(':n', $name);
        $stmt->execute();
    ...
    private static function mapCsvHeader(array $header): array
    {
        ...
        // category 在 CSV 里是“分类名称”文本（导出端也写名称），统一映射到
        // 虚拟键 category，导入循环负责把它解析成 category_id —— 绝不把裸 id 当文本。
        if (isset($map['category_id'])) {
            $map['category'] = $map['category_id'];
            unset($map['category_id']);
        } elseif (!isset($map['category'])) {
            foreach ($header as $i => $cell) {
                $c = strtolower(trim($cell));
                if (in_array($c, ['分类', 'category', 'category_id'], true)) {
                    $map['category'] = $i;
                    break;
                }
            }
