...
class OrderItem extends Model
{
    ...
    /** 订单当前状态；查不到按 'pending'（未取消）处理，避免误释放。 */
    public function statusOf(int $orderId): string
    {
        $row = $this->db()->query('SELECT status FROM orders WHERE id = :id')
            ->bind(':id', $orderId)->single();
        return (string) ($row['status'] ?? 'pending');
    }

    /** 行 → 商品占用库存：product_id => 数量合计（没关联商品的快照行不占库存）。 */
    public static function heldByRows(array $rows): array
    {
        $held = [];
        foreach ($rows as $r) {
            ...
            $held[$pid] = (float) ($held[$pid] ?? 0) + max(0, (float) ($r['quantity'] ?? 0));
        }
        return $held;
    }

    /** 一组行在给定订单状态下占用的库存（cancelled = 不占用）。 */
    private function commitment(array $rows, string $status): array
    {
        return $status === 'cancelled' ? [] : self::heldByRows($rows);
    }

    /**
     * 按 (新占用 − 旧占用) 对商品库存做净调整。
     * 方案 A 语义：订单只要不是 cancelled 就占用库存；改状态/改行/删单都收敛到这里算差值。
     */
    public function applyStockDelta(array $old, array $new): void
    {
        $products = new Product();
        foreach (array_unique(array_merge(array_keys($old), array_keys($new))) as $pid) {
            // 库存 = 现有值 − 净占用增量：占用量从 0→n 要扣 n；释放 n→0 要加回 n。
            $delta = (float) ($old[$pid] ?? 0) - (float) ($new[$pid] ?? 0);
            if (abs($delta) < 1e-9) {
                continue;
            }
            $products->adjustStock((int) $pid, $delta);
        }
    }

    /**
     * 把某订单当前占用的库存加回去（删订单/删客户连带删订单前调用）。
     * 已取消订单不占用、不误加；$force=true 时忽略当前状态直接按行释放
     * （用于 AI 刚把订单改成 cancelled 后这一瞬间——库里已是 cancelled，但行仍占用着）。
     */
    public static function releaseOrderStock(int $orderId, bool $force = false): void
    {
        $oi = new OrderItem();
        if (!$force && $oi->statusOf($orderId) === 'cancelled') {
            return;
        }
        $oi->applyStockDelta(
            self::heldByRows($oi->byOrder($orderId)),
            []
        );
    }

    /** 让某订单按当前明细重新占用库存（cancelled → 恢复成其它状态时调用）。 */
    public static function commitOrderStock(int $orderId): void
    {
        $oi = new OrderItem();
        $oi->applyStockDelta(
            [],
            $oi->commitment($oi->byOrder($orderId), $oi->statusOf($orderId))
        );
    }
     ...
     */
    public function syncItems(int $orderId, array $items, ?string $prevStatus = null): void
    {
        // 库存（方案 A：订单只要不是 cancelled 就占用库存）——
        // 旧行与旧状态必须在删除明细前拍下，才能算出"净变化"而不是重复扣/漏扣。
        // 同一次请求里改了状态的调用方（编辑表单把订单改成取消/恢复）必须传 $prevStatus；
        // 其余场景（新建订单、只改行）省略，默认取当前库内状态（这些流程不碰状态）。
        $oldRows = $this->byOrder($orderId);
        $oldStatus = $prevStatus ?? $this->statusOf($orderId);
        ...
        // 库存净变化 = 新占用 − 旧占用（含状态切换带来的释放/重新占用）。
        $newStatus = $this->statusOf($orderId);
        $this->applyStockDelta($this->commitment($oldRows, $oldStatus), $this->commitment($items, $newStatus));
    }
