...
// ---- Backup (整库+附件的导出/恢复，仅管理员) ----
// 导出走 POST：它把全库明文递出去，不能给任意页面一个自动提交就触发。
// download 是 GET，所以带 ?token= 自己验一道。
$router->get('/backup', 'BackupController@index');
$router->post('/backup/export', 'BackupController@export');
$router->post('/backup/snapshot', 'BackupController@snapshot');
$router->get('/backup/download', 'BackupController@download');
$router->post('/backup/snapshot/delete', 'BackupController@deleteSnapshot');
$router->post('/backup/verify', 'BackupController@verify');
$router->post('/backup/import', 'BackupController@import');
$router->post('/backup/history/clear', 'BackupController@clearLog');
