    ...
    public static function aiProviders(): array
    {
        ...
        foreach (AiClient::providers() as $key => $p) {
            $out[] = [
                ...
                // 输出长度的下限（思考型服务商：思考 token 与正文共用上限，太小会回空）
                'floor'     => (int) ($p['tokens_floor'] ?? 0),
        ...
        return [
            ...
            [
                ...
                'rules' => [
                    ...
                    '商品库 products 是主数据：商机与订单的明细行必须引用它里的商品（页面选择框与 AI 的 set_order_items 共用 OrderItem::normalizeRows()/Product::resolve()，只有一套规矩）；order_items 同时保留 product_name/sku/unit/unit_price 的成交快照——商品今天改价不能改写昨天已签的订单。引用可写编号 PROD-000007、SKU 或精确名称；对不上就整批拒绝并列出同名候选。被明细引用的商品删不掉（Ai 与页面都拒绝/改成停用），历史订单才始终看得懂自己卖过什么',
                    ...
                    '关联列（customer_id / deal_id / order_id / lead_id / product_id）的参数类型来自外键本身，Ai::fieldsFor() 不让字段注册表的语义类型覆盖它：deals 把 customer_id 登记成 int 曾使 create_deal 只收整数，而提示词教模型写 CUS-000007 —— 于是模型要么被“必须是整数”拒掉，要么猜一个数字，最后撞上 SQLSTATE 外键错误（用户看到一句数据库报错）。现在解析不出真实记录就在写库之前被拒，并提示先用 search_records 拿编号',
                    '同一个计划内可以“先建后挂”：create_customer 之后，后续动作写 customer_id:"@1" 引用第 1 步刚建的那条；@N 只能向前引用、类型必须对得上、被引用那一步必须无错（三条都在 AiDealChainTest 里），真实 id 由服务器在 execute 时填，模型猜不到也伪适不了。预览页用 Ai::argText() 把 @1 展开成「用第 1 步新建的客户」，确认闸门上必须让人看清挂给谁',
                ],
            ],
            [
            ...
            [
                'title' => '备份与恢复（后台一键 / 跨机转移）',
                'steps' => [
                    '全部数据只有两处：database/crm.sqlite（含 app_settings 里存的 AI Key）与 public/uploads/attachments/ 的文件（库里只存 filename，路径由 Attachment::uploadDir() 现算）',
                    '后台 备份与恢复（/backup，仅管理员）：一键导出 zip 到浏览器 / 生成服务器快照到 database/backups/ / 仅检查这个包 / 一键导入覆盖',
                    '「导出」与「生成快照」调的是同一个 Backup::createArchive()，包内容一字不差，区别只在落在哪：导出进你手里（服务器发完即删，不在列表里），快照留在 database/backups/（进列表、可保留 N 份、可直接当恢复来源）。防整机没了靠导出+异地，防写错靠快照+回滚点',
                    '备份包固定布局：manifest.json + database/crm.sqlite + uploads/attachments/<文件> + 可选 .env；导入时只认这四类条目，其余一律丢弃',
                    '跨机转移：旧机「生成快照」→ 把 crm-backup-*.zip 拷到新机的 database/backups/ → 新机选「从服务器快照恢复」→ 输确认词「覆盖导入」',
                    '命令行与 cron 方案见 DEPLOY.md 第 7 节（同一份约束：WAL 下不要 cp）',
                ],
                'rules' => [
                    'WAL（journal_mode=wal）下不得 cp 正在使用的 .sqlite：会漏掉 -wal 里已提交的事务。Backup::snapshotSqlite() 用 SQLite3::backup，没有 ext-sqlite3 时退到 VACUUM INTO，两者都是一致性快照',
                    'ZipArchive 与 PharData 双路径：Windows 的 php.ini 默认注释掉 extension=zip，只有 phar 也能完整导出导入（实测 PharData 读写外部 zip 可行；但 phar:// 遍历嵌套条目在 Windows 上不靠得住，所以解包一律 extractTo 到临时目录再走文件系统）',
                    '恢复是「原地覆写」而不是换文件：Windows 上被 PDO 打开着的库删不掉也改不了名，而本请求在 requireAuth 时已经持着这个连接；SQLite3::backup 反向写入后顺带 wal_checkpoint(TRUNCATE)',
                    '覆写前必定先生成 crm-pre-restore-*.sqlite 回滚点，并且这类文件永不被保留策略清理（prune 只动 crm-backup-*）；回滚点也必须能当恢复来源，并在下拉里标「回滚点·仅数据库」—— 刚导错的那一刻最短的退路就是它，而把它隔在上传后面（本机 upload_max_filesize 常是 2M）等于堵死。它只含库不含附件，所以标签必须说清',
                    '恢复是整体替换：备份之后新增的行会消失，备份里没有的表也会消失（BackupTest 用一张 zz_only_live 表钉住这个语义）。要“合并两边数据”请去写迁移脚本，不是点这个按钮',
                    '附件默认只新增/同名覆盖（merge）；「整目录替换」必须显式勾选且只删包里没有的文件 —— 附件只有一份真相，误删无法回填',
                    '.env 永不就地覆盖：最多另存为 .env.restored-时间戳 给人比对；导出默认也不含 .env（里面有密钥）',
                    '恢复后会 DELETE FROM remember_tokens 并清当前 remember cookie：旧设备的凭据不能走进恢复后的数据（也因此目标机的 remember_tokens 比源机少 1 行是正常的）',
                    '结构跟不上时由 migrate.php --no-demo 兜住（旧备份 + 新代码可直接起）；--no-demo 是必需的：否则 INSERT OR IGNORE 的演示数据会混进真库',
                    '快照名只过一条正则（crm-(backup|pre-restore)-Ymd-His-<6hex>.(zip|sqlite)）+ basename()：下载/删除/恢复都不接受任何路径形式的外部输入；history.jsonl 操作留在库外，因为恢复本身会换掉库里的痕迹',
                    '上传受 upload_max_filesize / post_max_size 限制（超限的 POST 会被 PHP 整块丢弃，所以 import/verify 在验 CSRF 前先 guardPostSize 并说人话）；大包请走「从服务器快照恢复」',
                ],
            ],
        ];
    }
