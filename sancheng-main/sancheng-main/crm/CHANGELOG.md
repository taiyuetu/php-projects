...
- **客户详情页的跟进记录补齐 CRUD（新增 / 查看 / 编辑 / 删除）**：此前 `follow_ups` 只有一个
  ...
  - `PUT /customers/{id}/follow-ups/{followUpId}` 与 `DELETE /customers/{id}/follow-ups/{followUpId}`；
    浏览器不直接发这两种方法，表单按本项目既有约定用隐藏字段 `_method` 伪造（与客户的改/删同一套）。
  - 编辑用小弹窗回填原值（类型 / 标题 / 描述 / 下一步行动 / 下次跟进日期），字段与新增表单同口径；
    日期入库写法不一（`2026-10-01`、带时分、AI 写的中文相对日期）统一经 `appDateTime()` 再回填，
    否则 `<input type="date">` 拿到带空格的写法会显示为空，用户只改个标题就把日期静悄悄清掉。
  - 改/删前用 `FollowUp::findForCustomer()` 校验这条记录确实属于 URL 里的客户 —— 跟进 ID 是
    自增的，不校验就能拿一个客户的页面改动另一个客户的记录。
  - 校验规则按本项目约定进 `FollowUp::$fields`（标题必填、类型必须是 CHECK 约束里的四个值之一），
    控制器只做 `sanitizeInput()` 委托；类型的中文名与徽章配色也收进 `FollowUp::TYPES`，
    新增表单、列表、弹窗、校验文案共用一处声明（以前视图里手写的那份副本删掉）。
    故意没有把 `follow_ups` 登记进 `Fields::REGISTERED`：那会改掉关键词搜索列的口径
    与 AI 侧参数文案，而这次只需要页面表单的校验。
  - `HttpSmokeTest` 新增用例钉住整条链路（含「弹窗必须回填原值」「跨客户改/删必须被拒」
    「空标题不落库并给中文提示」），并把新建的 destroy 端点也纳入 CSRF 419 用例。

- **线索分类：来源分类（主数据）+ 星级 A/B/C/D** —— 与「商品 / 商品分类」同构：
  `categories` 表新增 `type` 列（`product` / `lead_source`），一套主数据表管两类分类；
  `leads` 新增 `source_category_id`（FK，删分类置空）与 `grade`（CHECK 只允许 A/B/C/D）。
  - **星级四级含判定标准**：A 类（热线索）明确需求+预算+近期采购，可直接转商机；
    B 类（温线索）有需求、预算待定、1–3 个月考虑；C 类（冷线索）潜在需求、长期培育；
    D 类（无效线索）信息错误/非目标/拒绝沟通，归档放弃。取值、中文名、徽章配色
    全在 `Lead::gradeOptions()/gradeDescriptions()/gradeBadgeClass()` 声明一处，
    表单下拉、列表徽章、星级筛选、详情页、判定标准说明卡共用同一份；
    `grade` 标 `strict` —— 非法值在校验层报中文错，而不是撞 DB CHECK 抛 500。
  - **分类管理页改为双标签页**（商品分类 / 线索来源分类），`?type=` 区分，
    统计列随类型显示商品数或线索数；类型创建后不可改（防引用关系错乱）。
    预置 7 个来源分类种子（展会 / B2B 平台 / 官网询盘 / 社交媒体 / 老客户推荐 /
    广告投放 / 其他）。
  - **线索列表新增星级筛选按钮与来源分类下拉**，表格新增星级徽章列、来源小字分类名，
    底部附判定标准说明卡；分页与筛选互相保持参数。
  - **侧边栏「线索」与「商品」改为同构的二级菜单**（线索列表 / 线索来源分类、
    商品列表 / 商品分类），并区分 `/categories?type=lead_source` 该高亮哪一组。
  - 迁移口径踩坑记录：基线 `schema.sql` 每次都会自愈式重放，因此**引用新列的种子与索引
    不能写进基线**（老库那时还没这一列，实测报 `no such column: type`）。按 public_code
    （006/007）的既有模式拆成两个增量：`022` 纯加列（新库自动跳过）、`023` 建索引+种子
    （幂等，两类库都跑）。
...
- **侧边栏二级菜单：一级菜单点一下 = 跳转列表页 + 展开子菜单。** 原来
  ...
  文字链接负责跳转（列表页判定 active 时子菜单自然带 `show` 展开，一次点击两件事都成），
  右侧箭头按钮只负责折叠/展开（不影响当前页面）。
  - 整行 pill 的 hover/active 高亮改由 `.nav-group` 容器承担，视觉与原来一致；
    箭头在 `aria-expanded="true"` 时旋转 180°，并用 `aria-controls` 关联子菜单。
...
### Added
- **商机新增「备注」列（`deals.notes`，增量 020 + 同步基线）**：`customers` / `leads` / `orders` 一直都有
  `notes`，商机是四张业务表里唯一没有的 —— 于是「把询价细节记在商机上」无处可写，AI 也只是照实报
  「不接受参数 notes」。按本项目的约定加字段只改注册表一处（`Deal::$fields` 标 `'form' => true`），
  表单的扩展字段自动区、AI 参数清单、字段校验就全通了；详情页加了一行显示（有内容才显示）。
  - **备注故意不进关键词搜索**：`deals` 的搜索列是手写死的「标题 + JOIN 的客户名」（客户名是主用法），
    把长文本塞进去会让这个主用法噪起来；询价细节的正当归处是商机详情页的跟进记录。
    `DealTest::test_deal_notes_round_trip_and_wiring` 把这个决定钉住了（写得到的东西结果行里读得到，
    但拿号码当关键词搜商机必须是 0 命中），免得下一个人以为是漏配。
  - 提示词实测 8328 → 8340（`notes` 出现在 create_deal / update_deal 两行参数里），仍 < 8350 上限。
- **线索新增「微信」列（`leads.wechat`，增量 019 + 同步基线）**：`customers` 一直有 `wechat`，
  `leads` 却没有 —— 于是询盘里最常留的微信号**在线索阶段就已经丢了**：「线索转商机」建客户时无处可拷
  （LeadController@convert 现在会拷它），线索表单与搜索也都没有它。现在表单有输入框、
  `Lead::$fields` 有登记（因此 AI 也能写这一列，字段清单本来就从表结构生成），搜索列一并加上
  （`FieldsTest` 的历史清单同步更新并写明原因：用户拿一个微信号回来搜，搜不到等于线索丢了）。
- **服务端会把「拿线索挂商机」的计划改写成「新建客户 + 新建商机」**（`Ai::routeLeadIntoCustomerForDeal`）：
  上一版的 `@N` 只解决了“商机挂到刚建的**客户**”，但真实模型面对「…询价…新建商机」常常两步都想要 ——
  先 `create_lead`，再让商机写 `customer_id:"@1"`，于是整批被类型校验拒掉（用户原报障：
  「客户 的 @1 新建的是线索，与这里的类型不匹配」）。系统里“线索转商机”按钮本来就在做
  “从线索联系方式建客户再挂商机”，所以这是把页面既有的语义搬到 AI 这条路上，不是发明新规矩：
  那一步由 `create_lead` 变成 `create_customer`，**序号不变**（后面的 `@1` 仍指它），
  联系方式（含微信）一并带过去，`conversion_time` 落当前上海时间；线索独有的列
  （title/lead_time/source/status/value）不会混进客户表。代价说清楚：这一条不再留线索记录，
  预览页与回答里都写明，用户在确认闸门上看得到、可反悔。只建线索、或商机挂真实 `CUS-` 编号，一律不改写。
- **编号认错表的报错会说清“这两张表不一样”**：商机 `customer_id` 写了 `LEAD-000004` 时，
  过去只说「找不到对应记录」，模型会换个编号再试一轮；现在直接说
  「是线索的编号，这里要的是客户编号…不存在就同一计划分两步：先 create_customer，下一步写 "@N"」。
- 提示词预算 **8300 → 8350**：`leads.wechat` 是新列（参数清单由表结构生成，自动进提示词），
  并给规则 5b 补了一句「**商机只认客户，不能挂在线索上**」，实测 8328。三处上限断言的注释同步记了原因
  （沿用 8100→8300 那次“新增列就写清为什么”的做法）。
- **同一个计划里可以“先建、后挂”：`@N` 引用刚新建的那条**（AI 助手）。
  一句「墨西哥客户发来轮毂单元400套询盘…新建商机」过去无法完成：商机必须属于某个客户，
  而这个客户**还不存在**——库里查不到，模型就只能猜编号（见下方 Fixed）。现在模型出两步：
  - 第 1 步 `create_customer`（电话 / WhatsApp / 微信 / 国家一并写进去），
    第 2 步 `create_deal` 写 `customer_id:"@1"`；`@N` 表示“第 N 步刚建出来的那条”，
    **真实 id 由服务器在执行时填**，模型既猜不到也伪适不了（这与“编号不得由调用方提供”同一条规矩）。
  - 校验只允许**向前引用**（`@N` 必须早于本动作）且**类型要对得上**（@1 建的是商机就不能当客户用）、
    被引用的那一步必须本身无错——三条都有断言（`AiDealChainTest`），因为任何一条松掉都等于
    把商机挂到别人名下。
  - 预览页把 `@1` 展开成「@1（用第 1 步新建的客户）」：确认闸门上必须让人看清这单会挂给谁，
    只写 `@1` 等于让人闭眼签（`Ai::argText()`）。
  - 提示词规则 1b 同时补上「**点名要什么就做什么**」：用户说“新建商机”就出 `create_deal`，
    不许自作主张退成线索——上一版规则只教了“询盘一律先建线索”，于是点名要商机也被改成了线索。
    预算：提示词实测 8295 字（上限 8300，为塞进 5b 把规则 5 与 1b 的冗余表述压掉了；
    服务器已经会拒错引用，所以“为什么不能猜编号”这种解释从提示词里删掉不损失任何保证）。
- **后台一键备份 / 恢复（`/backup`，仅管理员）** —— 目标就一句话：要换机器、要把数据带走，不必再登
  服务器 `cp`（而 `cp` 正在用的 sqlite 恰好是这个栈上唯一会“备份成功但数据不全”的操作）。
  - `app/models/Backup.php`（能力、快照、打包、校验、恢复、留痕）+ `app/controllers/BackupController.php`
    + `app/views/backup/index.php`，共 8 条路由（`/backup`、`export`、`snapshot`、`snapshot/delete`、
    `download`、`verify`、`import`、`history/clear`）；侧边栏与顶栏菜单只管理员可见。
  - **导出**：一个 zip 里装 `manifest.json` + `database/crm.sqlite` + `uploads/attachments/*`（`.env` 默认不含，
    要的人得显式勾，因为它里面有 AI Key）。manifest 带各表行数、每个附件的 size+sha256、已应用迁移台账、
    导出版本与执行人 —— 备份包自己能说清它是什么，也能被对账。
  - **快照**：同内容落 `database/backups/`，可选“保留最近 N 份”自动清理；`crm-pre-restore-*` 永不清理。
  - **恢复**：先校验再写（文件头、`PRAGMA integrity_check`、核心表存在、manifest 行数/大小/sha256 对账、
    包内格式版本），然后先生成回滚快照、再覆写库、再放回附件、最后自动跑 `migrate.php --no-demo`。
    来路只能两种：上传包，或选一个已在 `database/backups/` 里的快照（后者不受 PHP 上传限制，也是回滚最短的路）。
  - **三个丌不起眼的硬约束**（都是实测，不是保守主义）：
    1. `journal_mode=wal` → 不 cp 主文件（会漏 -wal 里已提交的），一律 `SQLite3::backup`，没有 ext-sqlite3 时退 `VACUUM INTO`；
    2. Windows 上被 PDO 打开的库删不掉也改不了名，而恢复请求在 `requireAuth` 时已经持着这个连接
       → 恢复是「把备份反向写进正在使用的库」，不是换文件；
    3. 本机 `php.ini` 默认注释掉 `extension=zip`（ZipArchive 不可用）→ 打包/解包都写了
       ZipArchive / PharData 双路径；PharData 在 Windows 上遍历外部 zip 的嵌套条目会 stat 失败，
       所以解包统一 `extractTo` 到临时目录再走文件系统，不赌 `phar://`。
  - **安全考虑**：全部动作 `requireRole('admin')` + CSRF；导出走 POST（它把全库明文递出去，不能给任意页面一个自动提交就触发）；
    下载是 GET 所以带 `?token=`；快照名只过一条正则 + `basename()`，因此下载/删除/恢复的参数永远读不到目录外的文件；
    解包只认四类白名单路径（含 `.php` / 子目录 / `..` / 隐藏文件全部丢弃）；确认词“覆盖导入”必须手输；
    恢复后清空 `remember_tokens` 并踢当前 remember cookie（旧凭据不能走进恢复后的数据）；
    `.env` 永不就地覆盖（最多另存为 `.env.restored-*`）；附件默认 merge，整目录替换要显式勾选。
    快照与回滚点都能当恢复来源（页面上标清谁含附件、谁只含库）；“导出”与“生成快照”是同一个
    `Backup::createArchive()`，差别只在包落在哪 —— 落在你手里防的是整机没了，落在服务器上是防写错。
  - **超过 `post_max_size` 的 POST 会被 PHP 整块丢弃**（$_POST 与 $_FILES 全空），那时 CSRF 一定失败、
    只能报“表单无效”，管理员完全看不出是体积问题 —— 所以 import/verify 在验 CSRF 前先 `guardPostSize()` 说人话，
    并把两条出路（调大 ini / 走服务器快照）写在同一句里。
  - 操作留痕写在 `database/backups/history.jsonl`（**库外**）：写进库里的话，下一次恢复会把“恢复过”这个事实本身一起抹掉。
  - `config.php` 新增 `UPLOAD_PATH` / `BACKUP_PATH` 两个可覆盖常量（同 `DB_PATH` 一条规矩），
    `Attachment::uploadDir()` 改读前者；`.gitignore` 排除 `database/backups/*`（留 `.gitkeep` 与 `.htaccess`）。
  - 测试：`tests/cases/BackupTest.php`（19 项）+ `HttpSmokeTest` 新增一项 HTTP 往返（导出真拿到 zip →
    生成快照 → 改数据 → 错确认词无效 → 对确认词真的回到过去 → `../../.env` 当快照名递进去也不行）。
    其里两条故意钉住语义：“恢复是整体替换”（备份后建的表也会消失）、“回滚点真的能拿来回滚”。
    HTTP 往返里还钉了一条 UI 约束：`crm-pre-restore-*.sqlite` 必须出现在 `<option value="…">` 里
    （不是“页面上出现过这个名字”—— 表格行也印它，那样框不住就会在回归时假绿，已实测验证过这个断言会红）。
  - `tests/bootstrap.php` 现在把 `UPLOAD_PATH`/`BACKUP_PATH` 也隔离到 temp（否则备份用例会往仓库里写 zip、
    往真附件目录里写文件），并在退出时扫掉这两个临时目录。
...
- **线索 / 商机单页**：客户早已有 `/customers/{id}` 查看页，这次补上 `GET /leads/{id}` 与
  ...
  - 线索单页：联系信息、来源/预估金额/线索时间/负责人、流失原因（已流失时）、已转商机后关联客户直达链接，
    顶栏按状态提供「转为商机」/「重新激活」+ 编辑；
  - 商机单页：金额/客户/预计成交/负责人/阶段时间、**关联订单列表**（点单号直达订单页）、**附件面板**
    （上传/删除复用 `_attachments.php`）；已归档提供「恢复」，成交但无订单提供「创建订单」。
  - `LeadController::show()` / `DealController::show()`：记录不存在时提示并回列表。
- **客户列表「转化时间」列**：客户列表在「状态」与「负责人」之间新增转化时间列，显示为纯日期
  （与客户详情页同一 `formatDate('Y-m-d')` 格式），空值显示 `—`。
- **商品分类成为独立主数据**：新增 `categories` 表（基线 `schema.sql`）与 `Category` 模型、分类管理页
  （侧边栏「分类」）。商品的分类在表单里变成下拉选择（只列出启用项），列表/CSV 筛选用 `category_id`。
  老分类文本由迁移 `014` 收编成分类行并回链。
- **记住密码（30 天免登录）**：登录页“保持登录状态”radio，勾选后签发 `remember_tokens`（库里只存哈希）
  并写入 HttpOnly cookie；下次无会话访问自动登录，token 每次使用后轮换；登出或不勾选即作废。

### Changed
- **商品库新增「库存」列**（`products.inventory`）：只改了 `Product::$fields`（`searchable` + `csv`）、
  基线 `schema.sql` 与增量 `database/migrations/016_add_inventory_to_products.sql`，当前库用
  `php database/schema-sync.php --table=products --apply` 对齐——字段清单由表结构生成，所以表单、
  关键词搜索、CSV 导入导出、AI 可写清单四处同时生效，视图零改动（这就是“一处声明、多处派生”）。
  副作用：AI 系统提示由 8084 → **8104** 字，顶红了三个用例共用的那一条“提示词体积守门”断言（上限 8100）。
  按本项目既有约定（见本版“预算上限 7900 → 8100”那条）把上限 **8100 → 8300**，三处同步修改：
  `AiTest::test_requests_are_bounded_so_answers_come_back_fast`、`AiBulkTest::test_the_prompt_teaches_the_query_then_act_pattern`、
  `AiPermissionsTest::test_the_tool_table_is_the_single_source_of_truth`。属真实能力增长，不是阈值漂移。
- `products.category_id`（`INTEGER REFERENCES categories(id)`）是迁移 `013` 的纯 ADD COLUMN（新库基线已含、
  自动跳过）；`idx_products_category_id` 与三个同步触发器在 `014` 里建 —— 与 `010` 同因：基线先重放、
  老库那时还没有这一列。`products.category` 文本列继续保留，由触发器随分类改名/改选/删除保持同步，
  所以列表显示、关键词搜索与 AI 读到的分类永远等于下拉里选的分类，不存在“两套真相”。
...
- AI 新增商品分类 CRUD 工具（`create_category` / `update_category` / `delete_category`）以及
  ...
  （本项目约定：模块真实增长时在 CHANGELOG 说明即可调整）。内置演示模型（离线 mock）与提示词的
  “结构速览”也把分类当独立主数据：演示模型支持“有哪些分类 / 新建分类 X”，避免再出现
  “商品为 0，分类属于商品属性”这类把分类和商品混为一谈的答复。
  分类没有 public_code 列，但 AI 搜索/删除路径现在给分类提供**派生引用 CAT-0000xx**
  （由 id 生成，无需改表），get_record/update/delete 均接受该引用或数字 ID——模型拿
  到的结果和其它记录一样“有编号可引用”，不再因为只见裸 id 而误判库内无分类。
  search_records 的 `tables` 在**校验层和执行层都接受别名**（categories/分类/商品分类…），
  删除/更新分类的 `category_id` 也可写 CAT-编号或分类名（name 唯一）；无法识别的范围会
  返回可用取值清单，而不是被静默忽略后让模型以为“表为空”。
  演示模型也修正："删除所有商品分类"不再被误判为删商品（先查 category 表），并能在第二
  轮按分类行生成 `delete_category`；系统提示词新增硬规则 7b：分类是独立主数据，不是
  商品表的 category 过滤列。
- **真正的病根（本轮才找到，已修）**：AI 上下文的“库内总数”快照只统计七张表，**不包含分类**；
  真实模型看到“商品 0”又看不到任何分类计数，就直接编出“categories 表为空”——之前的别名、
  CAT 引用都没错，但没解决“模型根本不知道有分类这张表”。现在 `contextDigest()` 把分类计数
  一并给出；另加服务端兜底：用户明明在说分类、而模型这一轮既没查库也没给动作时，系统
  自己真实执行一次 `search_records(tables:category, all:true)` 并把结果推回去再问一轮，模型不
  再能凭印象断言库内没有分类。已用假 transport 端到端验证：“瞎答表为空”的第一轮会被
  系统接管，第二轮拿到真实 3 条分类后正常出 3 步删除并执行成功。
- 客户「转化时间」显示统一为只含日期（列表页新增列与详情页均为 `Y-m-d`），不再带时分。

### 模型清单更新：智谱预设升级为 GLM-5.3-Flash (Model presets)
- **智谱 GLM 预设改为 `glm-5.3-flash`（默认）**，可选 `glm-4.7-flash`（免费档）与 `glm-5.2`；
  端点仍是 `https://open.bigmodel.cn/api/paas/v4`（OpenAI 兼容的按量付费端点）。旧预设里的
  `glm-4-flash` / `glm-4-plus` 已不是当期主力，一并换掉。GLM-5.3 的**文本** API 官方标注“即将上线”，
  所以不放进预设——预设里放一个还调不通的 id，管理员一保存就收 400（与 MiMo v2 已下线模型同一条教训）。
- **GLM-5.3 强制开启思考，快速模式只能换开关**：官方文档明说 `thinking.type=disabled` 会报错，
  DeepSeek / MiMo 那套“关掉思考”照抄过来就是 400。预设改为发 `reasoning_effort: low`
  （官方唯一的思考档位开关；默认 `max` 是深度思考，一句“新建线索”也要想很久）。
- **思考 token 与正文共用输出上限，所以预设新增 `tokens_floor`**：设置页里最小的 400/800 会全花在
  思考阶段，`content` 回一个空字符串（官方也建议 `max_tokens ≥ 1024`）。`AiClient` 现在按
  “用户设置 vs 服务商下限”取大者发送，智谱下限 2048；「不限制」（0）的语义不变，仍然不发这个参数。
  设置页在“最大回复长度”下方、`/help` 的服务商表都写明这件事（字段由 `AppMap::aiProviders()` 从
  同一份预设取，不再手写第二份）。
- 智谱预设同时打开 `json_mode`（`response_format: json_object`）：本功能的协议本来就是一个 JSON 对象，
  由服务端保证语法合法，能整片避开“带围栏/带解说的回复解析失败”。
- 新增 `AiTest::test_glm_53_flash_is_wired_without_trying_to_disable_thinking`（智谱的两条规矩：
  绝不发 `thinking.type=disabled`、输出长度下限真的生效），以及在预设 URL 清单里补上智谱端点。

### Fixed
- **“最大回复长度 = 不限制”其实一直是 800**：`AiClient::config()` 用 `?: 800` 兜默认值，而字符串
  `'0'` 在 PHP 里是假值——设置页那个“不限制（交给服务商，可能很慢）”的档位等于白给。现在只有
  空值（从未填过）才回落 800，`'0'` 真的变成 0（也就是不发送 `max_tokens`，交给服务商）。
- **AI 建商机报 `SQLSTATE[23000] FOREIGN KEY constraint failed`**（用户原报障：
  「墨西哥客户发来轮毂单元400套询盘，客户名字低压的…新建商机」执行失败）。两个原因叠在一起：
  - `Ai::fieldsFor()` 让**字段注册表的语义类型覆盖了外键应有的引用类型**：`Deal::$fields` 把
    `customer_id` 登记成 `'int'`，于是 `create_deal.customer_id` 变成普通整数——而提示词教模型
    “引用一律写 CUS-000007”。同一个列在 `create_lead` 上又是好的（`leads` 没注册表），
    **两张表两套规矩**才是最难查的地方。现在关联列一律保留引用类型（`Ai::LINK_COLUMNS` 是判据），
    商机/订单的 `customer_id`、`order_id` 等一律接受编号与真实 id，`value` 仍是 money、`stage` 仍是 enum
    （`test_link_columns_stay_references_in_registered_tables` 同时钉住这两面，防它整片被改坏）。
  - 校验只判“是不是整数”，不判“这行在不在”，所以模型猜的数字直通 `execute()` 撞上数据库约束，
    人看到的是一句 SQLSTATE。现在编号/记录解析不出来就在**写库之前**被拒，并说清该怎么办：
    「客户「999999」找不到对应记录（编号形如 CUS-000007…；请先用 search_records 拿到真实编号）」。
- **`PublicCodeTest` 的“造老库”夹具在 Windows 工作区里一直是坏的**（与上面的备份无关，顺手修了）：
  它用 `str_replace("...CASCADE,\n", ...)` 抹掉被删外键后留下的结尾逗号，而 `database/schema.sql`
  在 checkout 出来是 CRLF（git 索引里才是 LF），那个带 `\n` 的字面量永远匹不上 →
  `order_items` 变成 `..., )` → `SQLSTATE[HY000]: General error: 1 near ")"`。
  改用行尾无关的正则，与其它几条剥列的写法保持一致 —— 同一个“拿字面量赌行尾”的坑不再靠记性避。
...
- **AI `get_record` 查不到线索/商机**：`runDetail` 把 `args.id` 先 `(int)`（编号 `LEAD-000007` → 0），
  在编号解析逻辑之前就被“记录类型或 ID 不合法”拦下，AI 批量核对备注必然失败。重构为先校验类型、
