...
- **Backup & restore** (`/backup`, admins) — one click exports a zip of *everything* (`manifest.json` + a WAL-safe
  `crm.sqlite` snapshot + `uploads/attachments/*`, `.env` only if you ask), either to the browser or as a server-side
  snapshot in `database/backups/` with retention. Import takes a zip (or a bare `crm.sqlite`) from either source,
  **verifies before writing** (SQLite header, `integrity_check`, required tables, manifest row/sha256 reconciliation),
  then writes it **into the live database in place** (`SQLite3::backup` — no file swap, so no Windows lock games),
  **after** leaving a `crm-pre-restore-*.sqlite` rollback point that retention never touches. Restore is a full replace
  (rows added after the backup disappear, and so do tables the backup doesn't have); attachments merge by default and
  `migrate.php --no-demo` runs afterwards so an old backup lands on new code. `remember_tokens` is cleared so old
  cookies can't walk into restored data, and the action log lives in `database/backups/history.jsonl` — outside the DB,
  because a restore would otherwise erase the record of itself. Only the four whitelisted archive paths are ever read
  back (`manifest.json`, `database/crm.sqlite`, `uploads/attachments/<flat name>`, `.env`), which is what keeps a hand-made
  zip from writing `app/bootstrap.php` or a `.php` into the uploads dir
...
├── database/backups/          # 后台「备份与恢复」的快照与 history.jsonl（gitignore，.htaccess 拒访）
