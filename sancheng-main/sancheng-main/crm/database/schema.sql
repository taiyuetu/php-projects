...
-- ---------------------------------------------------------------
-- 1c. Remember-me tokens (记住登录，30 天内免登录)
-- ---------------------------------------------------------------
-- 存哈希后的 token；cookie 里放的是原值（只出现一次，永远不再落库）。
...
-- ---------------------------------------------------------------
-- 1d. Product categories (商品分类主数据)
-- ---------------------------------------------------------------
-- 商品表里仍保留 category 文本列：列表/AI/CSV 这些旧消费方都读它，
-- 由触发器随 category_id 同步（改名、改选、删除都会跟着变），
-- 所以它永远等于分类名，不会和下拉选择“两套真相”。
...
-- 注意：idx_products_category_id 与下面的分类同步触发器不放基线——旧库要等增量
-- 013 的 ALTER 才有 category_id 列，基线先重放会直接报 no such column（同 010 先例）。
...
END;
-- 商品分类同步触发器见增量 014（依赖 category_id 列，旧库基线重放时还没有）。
